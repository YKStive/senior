...
import com.youloft.util.IOListener;
...
public class IOUtils {
    ...
    public static String toString(byte[] input, String encoding) throws IOException {
        if (AppUtil.beforeSDK(Build.VERSION_CODES.GINGERBREAD)) {
    ...
    public static int copy(InputStream input, OutputStream output, IOListener ioListener) throws IOException {
        long count = copyLarge(input, output, ioListener);
        if (count > Integer.MAX_VALUE) {
            return -1;
        }
        return (int) count;
    ...
    public static long copyLarge(InputStream input, OutputStream output, final IOListener ioListener)
            throws IOException {
        return copyLarge(input, output, new byte[DEFAULT_BUFFER_SIZE], ioListener);
    }
     ...
     * @since 2.2
     */
    public static long copyLarge(InputStream input, OutputStream output, byte[] buffer, IOListener ioListener)
            throws IOException {
        long count = 0;
        int n = 0;
        while (EOF != (n = input.read(buffer))) {
            ...
            count += n;
            if (ioListener != null) {
                ioListener.copying(count);
            }
        }

        if (ioListener != null) {
            ioListener.copyOver(count);
        }
        return count;
    }
