 ...
 * @desc 收到的评论
 */
open class ReceivedCommentViewBinder(
    val onItemClick: (id: String, type: Int) -> Unit
) :
    ItemViewBinder<Comment, ReceivedCommentViewBinder.ViewHolder>() {
    override fun onCreateViewHolder(
        inflater: LayoutInflater,
        parent: ViewGroup
    ): ViewHolder {
        val root: View =
            inflater.inflate(R.layout.item_received_comment, parent, false)
        return ViewHolder(root)
    }

    override fun onBindViewHolder(
        holder: ViewHolder, item: Comment
    ) {
